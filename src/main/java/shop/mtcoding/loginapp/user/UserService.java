...
public class UserService {
    ...
    @Transactional
    public void 회원가입(String username, String password, String email) { // 이 회원가입은 일반 회원가입이기 때문에 프로바이더는 무조건 널
    ...
    public User 카카오로그인(String code) {
        // 1.  code로 카카오에서 토큰 받기(위임 완료) // 코드만으로는 위임이 된 것이 아닌데 코드를 통해서 카카오에서 토큰을 받은 것이 위임이 완료된 것이다.
        ...
        // 3. 해당 정보로 DB조회(있을 수, 없을 수)
        String username = "kakao_" + response2.getBody().getId();
        User userPS = userRepository.findByUsername(username);

        // 4. 있으면 findByUsername해서 조회된 유저 정보 리턴(세션에 그냥 넣음)
        if (userPS != null) {
